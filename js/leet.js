...
maxSlice([3, 2, -6, 4, 0]);
console.log(0 % 2);
function longestGap(N) {
    let binary = "";
    let counter = -1;
    let max = 0;
    function recursive(number, base) {
        if (number < 1) {
            return 0;
        }
        if (number % base > 0) {
...
function calculateDate(priceArray, dateArray) {
    ...
    const finalArray = paymentCountArray.filter((item) => item.count > 2 && item.total > 99);
    totalMoney = totalMoney + -5 * (paymentCountArray.length - finalArray.length);
    console.log(totalMoney);
    return totalMoney;
}
calculateDate([100, 100, 100, -10], ["2020-12-31", "2020-12-22", "2020-12-03", "2020-12-29"]);
calculateDate([180, -50, -25, -25], ["2020-01-01", "2020-01-01", "2020-01-01", "2020-01-31"]);
calculateDate([1, -1, 0, -105, 1], ["2020-12-31", "2020-04-04", "2020-04-04", "2020-04-14", "2020-07-12"]);
calculateDate([100, 100, -10, -20, -30], ["2020-01-01", "2020-02-01", "2020-02-11", "2020-02-05", "2020-02-08"]);
function renamePictures(string) {
    const picturesArray = [];
    const resultArray = [];
    let currentCity = "";
    ...
    picturesArray.forEach((element) => {
        ...
        let finalString = `${element.city}${element.personalIndex < 10 && currentCityIndex > 10
            ? `0${element.personalIndex}`
            : element.personalIndex}.${element.extension}`;
        resultArray[element.index] = finalString;
    });
    console.log(resultArray.join("\n"));
}
renamePictures("photo.jpg, Warsaw, 2013-09-05 14:08:15\njohn.png, London, 2015-06-20 15:13:22\nz.png, Warsaw, 2012-02-29 22:13:11");
