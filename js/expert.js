...
function isExactlyThree(n, target = 3) {
    if (n < 1) {
...
function handleSurname(surname) {
    ...
    const consonantsInSurname = surnameArray.filter((letter) => !vowels.split("").includes(letter));
    if (consonantsInSurname.length > 2) {
        return consonantsInSurname.slice(0, 3).join("");
    }
    const vowelsInSurname = surnameArray.filter((letter) => vowels.split("").includes(letter));
    return `${consonantsInSurname.join("")}${vowelsInSurname[0]}`;
}
function handleName(name) {
    const nameArray = name.toUpperCase().split("");
    const consonantsInName = nameArray.filter((letter) => !vowels.split("").includes(letter));
    const vowelsInName = nameArray.filter((letter) => vowels.split("").includes(letter));
...
function freqCount(nestedArray, element) {
    const finalResult = [];
    function recursive(array, level) {
        let tempArray = [];
...
function getItemsAt(array, condition) {
    const result = [];
