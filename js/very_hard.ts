...
function additivePersistance(n: number): void {
  let iteration: number = 0;

  if (n < 9) {
    iteration = 0;
    console.log(
      `It takes ${iteration} iteration to reach a single-digit number.`
    );
    return;
  } else {
    runIteration(n);
  ...
  function runIteration(integer: number): void {
  ...
  function checkNos(number: number): void {
    if (number > 9) {
      runIteration(number);
    } else {
      console.log(
        `It takes ${iteration} iterations to reach a single-digit number.`
      );
    }
  }
}

function multiplyPersistance(n: number): void {
  let iteration: number = 0;
  ...
  function checkNos(number: number) {
    if (number > 9) {
      runIteration(number);
    } else {
      console.log(
...
function numberToArray(num: number): Array<number | string> {
  let newArray: Array<string | number> = num.toString().split("");
...
function actualMemorySize(size: string): void {
  let nos: number = parseInt(size);
  let string = size.slice(size.length - 2, size.length);
  let totalSize: number = nos - nos * 0.07;
...
function balanced(word: string): void {
  let lettersObject = {};
  lettersArray.forEach((item, index) => {
    lettersObject = { ...lettersObject, [item]: index + 1 };
  });

  let wordArray: Array<string> = word.split("");
  let middle: number = Math.floor(wordArray.length / 2);
  let firtsPart: Array<string> = wordArray.slice(0, middle);
  let secondPart: Array<string> = wordArray.slice(
    wordArray.length - middle,
    wordArray.length
  );
  let firstPartTotal: number = 0;
  let secondPartTotal: number = 0;
...
function combineObject(obj1: {}, obj2: {}): void {
  let newObject: {} = { ...obj1, ...obj2 };
  Object.entries(obj1).forEach((item) => {
    if (obj2[item[0]]) {
      newObject = { ...newObject, [item[0]]: obj1[item[0]] + obj2[item[0]] };
    }
  });
...
combineObject(
  { pocketMoney: 1000, data: 3500, food: 1000 },
  { data: 3500, transport: 500, food: 1000 }
);
...
function periodic(num: string): void {
  let baseString: string = num;
  let length: number = num.length;
  let iteration: number = 1;
  let periodicArray: Array<string> = [baseString];
  ...
  function checkArray(string: string) {
...
function windowMaxes(array: Array<number>, windowLength: number): void {
  let newArray: Array<number> = [];
  let maxIteration: number = array.length - windowLength;
...
function getHashTags(headline: string): void {
  console.log(headline);
  let array: Array<string> = headline.split(" ");
...
function maxPossible(num1: number, num2: number): void {
  let firstArray: Array<string | number> = num1.toString().split("");
  let secondArray: Array<string> = num2.toString().split("");
  firstArray = toNumArray(firstArray);
  let secondNumArray: Array<number> = secondArray.map((item) => {
  ...
  function toNumArray(arr: Array<string | number>): Array<number | string> {
...
function lemonade(price: Array<5 | 10 | 20>) {
...
function trackRobot(...args: number[]) {
  let pos = 0;
  let coordinates = [0, 0];
