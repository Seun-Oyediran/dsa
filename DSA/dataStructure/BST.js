"use strict";
class node {
    constructor(data, key, left, right) {
        this.data = data;
        this.key = key;
        this.right = right;
        this.left = left;
...
class TestBst {
    ...
    insert(data, key) {
        function inserRec(root, data, key) {
            if (root === null) {
    ...
    find(key) {
        function findRec(key, node) {
            ...
            if (key < (current === null || current === void 0 ? void 0 : current.key)) {
                current = current.left;
    ...
    remove(key) {
        ...
        function removeNodeRec(key, node) {
            let current = node;
            if (current === null)
                return null;
            if (key === current.key) {
